...
public interface TDeviceDao extends BaseJpaDao<TDevice> {
    ...
    @Query(value = "select t1.* from t_device t1 join t_enterprise t2 on t1.enterprise_id=t2.id where t1.delete_flag=0 and t2.name like CONCAT('%',?1,'%') and t1.`status` like CONCAT('%',?2,'%') AND t2.area_id like CONCAT('%',?3,'%') limit ?4,?5",nativeQuery = true)
    List<TDevice> getPageListBySqlAndCondition(String enterprise, String status, String areaId,int pageNum,int pageSize);

    @Query(value = "select count(0) from t_device t1 join t_enterprise t2 on t1.enterprise_id=t2.id where t1.delete_flag=0 and t2.name like CONCAT('%',?1,'%') and t1.`status` like CONCAT('%',?2,'%') AND t2.area_id like CONCAT('%',?3,'%') ",nativeQuery = true)
    ...
    @Query(value = "select t2.`name`,t2.address,t2.contact,t2.contact_mobile as contactMobile,t2.env_contact as envContact,t2.env_contact_mobile as envContactMobile,t1.status,t1.last_upload_time as lastUploadTime,t1.lampblack,t1.temp,t1.humidity,t1.fan_elec as fanElec,purifier_elec as purifierElec,case WHEN lampblack > lampblack_standard THEN 3 WHEN lampblack > lampblack_warning THEN 2 else 1 end as lampblackStatus,fan_status as fanStatus,purifier_status as purifierStatus,t2.latitude,t2.longitude from t_device t1 LEFT JOIN t_enterprise t2 on t1.enterprise_id =t2.id  LEFT JOIN t_area t3 on t2.area_id =t3.id where t2.area_id in ?1 and t2.latitude>0 and t2.longitude>0 ",nativeQuery = true)
    List<Map> getDeviceListByAreas(String areas);
}
