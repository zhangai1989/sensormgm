...
public interface TUserDao extends BaseJpaDao<TUser> {
    ...
    @Query(value = "select count(0) from t_user t1 LEFT JOIN t_enterprise t2 on t1.enterprise_id=t2.id LEFT JOIN t_area t3 on t1.area_id=t3.id where t1.level <=1  and t1.area_id like CONCAT('%',?2,'%')",nativeQuery = true)
    int getTotalBySqlAndCondition(int levelCondition, String areaIdCondition);
