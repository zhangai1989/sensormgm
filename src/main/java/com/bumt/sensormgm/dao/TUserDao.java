...
public interface TUserDao extends BaseJpaDao<TUser> {
    ...
    @Query(value = "select t1.area_id as areaId,t1.enterprise_id as enterpriseId,t1.login_name as loginName,t1.cname,t1.email,t1.mobile,t1.create_time,t1.creator_name as creatorName,t1.modifier_name as modifierName,t1.modify_time ,t2.`name` as enterpriseName,t3.`name` as areaName from t_user t1 LEFT JOIN t_enterprise t2 on t1.enterprise_id=t2.id LEFT JOIN t_area t3 on t1.area_id=t3.id where t1.level <=1 and t1.area_id like CONCAT('%',?1,'%') limit ?2,?3",nativeQuery = true)
    List<Map> getPageListBySqlAndCondition( String areaIdCondition, int start, int pageSize);

    @Query(value = "select count(0) from t_user t1 LEFT JOIN t_enterprise t2 on t1.enterprise_id=t2.id LEFT JOIN t_area t3 on t1.area_id=t3.id where t1.level <=1  and t1.area_id like CONCAT('%',?1,'%')",nativeQuery = true)
    int getTotalBySqlAndCondition(String areaIdCondition);
